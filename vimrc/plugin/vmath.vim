...
function! s:average (numbers)
    ...
    ""Added by JMC (use max instead of min decimals):
    "" Determine significant figures...
    "let max_decimals = 0
    "for num in a:numbers
    "    let decimals = strlen(matchstr(num, '[.]\d\+$')) - 1
    "    if decimals > max_decimals
    "        let max_decimals = decimals
    "    endif
    "endfor
    "" Adjust answer...
    "return max_decimals > 0 ? printf('%0.'.max_decimals.'f', avg)
    "\                       : string(avg)

    "Added by JMC (always use full 15 decimals, like a calculator would, let
    "the user figure out how many decimals are really significant figures):
    let min_decimals = 15
    return printf('%0.'.min_decimals.'f', avg)

    "Added by JMC (use whatever Vim returns as toString):
    ""return string(avg)

    "Commented by JMC (this asumes the input numbers are measurements, and
    "tries to not give more decimals than the original precision, so if you
    "have 4 significant figures, you should write 4.700 instead of 4.7 in the
    "input or it will trim your decimals. Clearly not always the case):
    "" Determine significant figures...
    "let min_decimals = 15
    "for num in a:numbers
    "    let decimals = strlen(matchstr(num, '[.]\d\+$')) - 1
