...
let s:CLEAR=0|let s:CHECK=1|let s:DRAW=2|let s:DRAWBLUE=3
let s:shs=7|let s:cols=7
let s:amblyopiaMode=1
...
fu! s:Init()
 ...
 se ve=all
 setl bh=delete noswf bt=nofile nf= gcr=a:lcursor0 nolz
 ...
 syn match Shape6 "$\$"
 hi ShapeBlue term=reverse ctermfg=Blue ctermbg=Blue guifg=Blue guibg=Blue
 syn match ShapeBlue "UU"
 hi ShapeRed term=reverse ctermfg=Red ctermbg=Red guifg=Red guibg=Red
 syn match ShapeRed "OO"
 ...
 "Agregado pregunta de si acelerar con cada nivel:
 let s:dontSpeedUp=confirm('Speed up every ten points?',"Yes\nNo")-1
 "Agregado pregunta de si poner las que flotan en rojo y las ya puestas en azul:
 let s:amblyopiaMode=confirm('Amblyopia Mode?',"Yes\nNo")-1
endf
 ...
 wh 1
  wh 1
   ...
   if !s:Put(b:y+1,b:x,b:pos,s:CHECK)
    cal s:Put(b:y,b:x,b:pos,s:DRAWBLUE)|brea
   en
