...
" Last Changed: 2024-05-30  (for v0.2. Previous v0.1 was 2007-11-07)
...
"   Initial release
" 0.2 2024-05-30 (JMC)
"   -Fixed a bug when calling :'<,'>Md5 or :'<,'>Sha1 and the first line number
"    was alfabetically less than the second line number.
"   -Also now a single line can be checksummed, instead of returning the whole
"    file checksum when only a single line is selected in the range.
" ==========================================================================
...
function! SHA1_sha1sum(first, last) "{{{
	let eol = Eol()
	if str2nr(a:first) <= str2nr(a:last)
...
function! MD5_md5sum(first, last) "{{{
	let eol = Eol()
	"Note: The args are strings when calling as :'<,'>Md5, so comparing
	"      them with < or <= was unpredictable until using str2nr() first.
	"      For instance, '6' was greater than '19'.
	" echo a:first  a:last  typename(a:first)  typename(a:last)
	if str2nr(a:first) <= str2nr(a:last)
		let [f, l] = [a:first, a:last]
	else
		let [f, l] = [1, '$']
